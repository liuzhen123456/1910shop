  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li><a href="{{url('/guan/index')}}">管理员</a><>
      </ul>
    </div>
  </div>
</nav>
...
@csrf
    ...
    <div class="form-group">
		...
		<div class="col-sm-8">
			<input type="text" class="form-control" name="goods_hao"value="{{$res->goods_hao}}" id="firstname" 
				   placeholder="请输入商品货号">
    ...
    <div class="form-group">
		...
		<div class="col-sm-8">
		<input type="radio" class="form-control" name="is_new_show" value="1"{{$res->is_new_show=="1"?"checked":""}}>是
        <input type="radio" class="form-control" name="is_new_show" value="2"{{$res->is_new_show=="2"?"checked":""}}>否
		</div>
	</div>
    <div class="form-group">
		<label for="lastname" class="col-sm-2 control-label">是否新品</label>
		<div class="col-sm-8">
		<input type="radio" class="form-control" name="is_new" value="1"{{$res->is_new=="1"?"checked":""}}>是
        <input type="radio" class="form-control" name="is_new" value="2"{{$res->is_new=="2"?"checked":""}}>否
		</div>
	</div>
	<div class="form-group">
		<label for="lastname" class="col-sm-2 control-label">是否精品</label>
		<div class="col-sm-8">
		<input type="radio" class="form-control" name="is_best" value="1"{{$res->is_best=="1"?"checked":""}}>是
        <input type="radio" class="form-control" name="is_best" value="2"{{$res->is_best=="2"?"checked":""}}>否
		</div>
	</div>
    <div class="form-group">
		...
		<div class="col-sm-8">
			<textarea type="text" class="form-control" name="goods_desc" id="lastname" 
				   placeholder="请输入商品描述">{{$res->goods_desc}}</textarea>
    ...
    <div class="form-group">
		...
		<div class="col-sm-8">
            ...
            @foreach($cate as $v)
            <option value="{{$v->cart_id}}"{{$v->cart_id==$res->cart_id?"selected":""}}>{{$v->cart_name}}</option>
            @endforeach
            </select>
		</div>
	</div>
	...
	<div class="form-group">
		<div class="col-sm-offset-2 col-sm-10">
			<button type="submit" class="btn btn-default">重新购买<tton>
		</div>
	</div>
</form>
